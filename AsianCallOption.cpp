...
OptionType AsianCallOption::getOptionType() const{
    return OptionType::Call; //Override the getOptionType function of the Option Class by a function that returns Call
}
double AsianCallOption::payoff(double average) const {
    return std::max(average - _strike, 0.0);  //For an asian Call the payoff is max(0,average - K)
}
