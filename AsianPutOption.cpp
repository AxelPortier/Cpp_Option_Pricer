...
OptionType AsianPutOption::getOptionType() const{
    return OptionType::Put; //Override the getOptionType function of the Option Class by a function that returns Put
}
double AsianPutOption::payoff(double average) const {
    return std::max(_strike - average, 0.0);  //For an asian Put the payoff is max(0,K - average)
}
