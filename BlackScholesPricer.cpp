...
double BlackScholesPricer::delta() const {
    ...
    if (_option->getOptionType() == OptionType::Call) { // Call
        return _option->isDigital() ? std::exp(-_interest_rate * _option->getExpiry()) * norm_pdf(d2) / (_asset_price * _volatility * std::sqrt(expiry)) : norm_cdf(d1);
    } else { // Put
        return _option->isDigital() ? -std::exp(-_interest_rate * _option->getExpiry()) * norm_pdf(d2) / (_asset_price * _volatility * std::sqrt(expiry)) : norm_cdf(d1)- 1.0;
    }
}

double BlackScholesPricer::norm_cdf(double x) const {return 0.5 * std::erfc(-x / std::sqrt(2.0));}
double BlackScholesPricer::norm_pdf(double x) const {return 1.0 / std::sqrt(2.0 * _PI) * std::exp(-0.5 * x * x);}
