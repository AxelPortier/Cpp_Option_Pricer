...
AsianOption::AsianOption(double expiry,const vector<double>& monitoringTimes)
	:Option(expiry),timeSteps_(monitoringTimes) //Constructor that takes expiry (from Option class), and monitoringTimes = (t1,...,tm)
{
...
const vector<double>& AsianOption::getTimeSteps() const {
	return timeSteps_; //To get the timeSteps = (t1,...,tm)
}
